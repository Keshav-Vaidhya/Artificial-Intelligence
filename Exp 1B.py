...
class Cell: 
 def __init__(self, position, location, max_val, min_val):  self.position = position 
...
def generate_cells(board): 
 ...
 if uboard[i][j] != 'X' and uboard[i][j] != 'O':  uboard[i][j] = Cell(uboard[i][j], [i,j], 0, 0)  uboard[i][j].max_val = max_val(board, [i, j])  uboard[i][j].min_val = min_val(board, [i, j])  # NOTE convert uboard[i][j] into list of maxval and  minval from objects 
...
def min_val(board, location): 
 ...
 minval -= check_vertical(board, location[1], 'min')  minval -= left_diagonal(board, location[0], location[1],  'min') 
...
def right_diagonal(board, row, col, u_type): 
 ...
 if board[i][abs(i-2)] == board[row][col]:  state = True 
...
def dispUboard(uboard): 
 ...
 for j in range(len(uboard[i])): 
 count += 1 
 if uboard[i][j] == 'O' or uboard[i][j] == 'X':  print(' ',uboard[i][j],end=' ')  else: 
 print(uboard[i][j],end=' ')  if count%3 == 0: 
 print('\n') 
def checkWin(board, sign): 
 if checkHorizontal(board, sign) == True:  return True 
...
def checkTie(board): 
 ...
 if board[i][j] == 'O' or board[i][j] == 'X':  filled += 1 
 if filled == 9: 
 return True 
 return False 
def checkDiagonal(board, sign): 
 ...
 for j in range(len(board[i])):  if board[0+j][2-j] == sign:  filled += 1 
 if filled == 3: 
 return True 
 return False 
def checkHorizontal(board, sign): # NOTE BUGGY so fix it  for i in range(len(board)): 
 if board[i][0] == sign: 
 filled = 0 
 for j in range(len(board[i])):  if board[i][j] == sign: 
 filled += 1 
 if filled == 3: 
 return True 
 return False 
def checkVertical(board, sign): 
 ...
 if board[0][i] == sign: 
 filled = 0 
 for j in range(len(board[i])):  if board[j][i] == sign: 
 filled += 1 
 if filled == 3: 
 return True 
 return False 
...
def checkCompatible(board, move, sign): 
 ...
 print("Please select an empty spot and try again.")  return False 
def computerDecision(board): 
 ...
 computer_decision = minimax_algorithm(uboard)  computer_decision = int(computer_decision) 
 if checkCompatible(board, computer_decision, 'O') == True:  if checkTie(board) == True: 
 dispboard(board) 
 play_again = input("\nThis is a tie game, to play  again enter any key, otherwise enter 'q' to quit.\nYour decision: ")  if play_again == 'q': 
 return 
 else: 
 board = [[0, 1, 2],[3, 4, 5],[6, 7, 8]]  GameInitializer(board) 
...
def playerDecision(board): 
 ...
 if checkCompatible(board, player_decision, 'X') == True:  if checkTie(board) == True: 
 dispboard(board) 
 play_again = input("\nThis is a tie game, if you want 
to play again enter 'p', to quit enter any key.\nYour decision: ")  if play_again == 'q': 
 return 
 else: 
 board = [[0, 1, 2],[3, 4, 5],[6, 7, 8]]  GameInitializer(board) 
...
def GameInitializer(board): 
 ...
 print("\nPlease enter 'c' or 'p' and try again.")  GameInitializer(board) 
def minimax_algorithm(ub): # should return a pos, such as 4, not  index[1,1] 
 ...
 if abs(ub[i][j][1]) == abs(ub[i][j][2]):  # NOTE if abs of max = abs of min, add 1 to  their sum. Why? because we want to win more more than limiting the  enemy 
 ...
 randnum = random.randint(0,len(redundant_optimal)-1)  return redundant_optimal[randnum] 
# NOTE play game here 
init_board = [[0, 1, 2],  
 [3, 4, 5], 
 [6, 7, 8]] 
